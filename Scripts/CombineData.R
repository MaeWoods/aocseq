...
marker.gene=c("IFNG","CRTAM","CD69","TNF","CD70","TNFRSF9")
mask=c("TRBV.","TRAV.","TRDV.","TRDC.","TRDJ.","TRDD.","TRGJ.","TRGD.")
...
save.dir="../RDS/ThreeHourStim.rds"
...
Gene_indUMO=match(marker.gene,row.names(Clonal_Obs[[k]]))
...
IMTCRs= vector(mode = "list", length = length(gex.path))
...
IMTCRs[[h]]=c(IntersectTCRs,Tailmono)
  ...
  for(j in 1:length(IMTCRs[[h]])){
    SizesC_Obj[[h]][j]=length(subset(tcrhash[[h]],cdr3_nt==IMTCRs[[h]][j] & productive=="true" & chain=="TRB")$barcode)
  }
...
clono.nucleotide.seq= vector(mode = "list", length = length(gex.path))
clono.aminoacid.seq= vector(mode = "list", length = length(gex.path))
for(k in 1:length(gex.path)){
