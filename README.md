...
The software combines probability and control theory with existing single cell analysis software for the classification of cells. Ultimately the aim is to provide a flexible framework that can be used to analyze multi-modal data. For a list of applications, see the section **Getting started** for the vignettes or the [slide deck](https://github.com/MaeWoods/aocseq/raw/main/SlideDeck.pdf) listing the functions. Version 0.1.0 can be used to quantify cells that are activated through the binding of their T cell receptors (TCRs) to the cognate antigens. Therefore, the tool provides a method to identify full length TCRs with specificity directed toward a target antigen, that can then be tested for use as transgenes. In a later update, functionality for time series analysis will be included to analyze changes in clone frequency in the TCR repertoire in the format of nucleic acid sequence productive frequency, the corresponding amino acid sequences and the total number of T cells. Currently, the data can be from multiple conditions or treatments, hashtagged and include multiple time points. 
