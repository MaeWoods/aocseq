...
**VRTrack** is software to analyze immunosequencing data written in R and Rcpp and can be used for the prediction of antigen specific T cells amongst the masses. The package was developed by the department of Cell and Gene Therapy at Baylor College of Medicine. For a full list of authors who contributed to the software design and application, please see the manuscript: ...
...
The TCR sequence is important to study because T cell specificity depends on a region of the TCR, known as the **CDR3** that is variable in sequence between individuals and cells. T cells can be grouped into clonotypes that share a common TCR CDR3 beta chain and this way, used to approximate the potential frequency of target specific T cells in the blood. Harnessing this heterogeneity in sequence between T cells for a quantitative analysis of T cell immunology has broad applicability to *in vivo* data analysis because clearing infection and disease depends on availability of immune cells (including T cells) with capacity to mount a response against infection. 
Immunosequencing is a PCR-based based method that exploits the capacity of high-throughput sequencing technology to characterize tens of thousands of TCR CDR3 chains simultaneously. **VRTrack** has been developed to efficiently analyse and annotate this data.

**Variable region Track** (**VRTrack**) is a software package of statistical tools that can be used to analyse, annotate and query clonotypes subject to amplification or reduction in frequency following antigen stimulation or between experimental conditions. **VRTrack** has initially been applied to Virus specific T cells (VSTs) that contain non specific bystander T cells in addition to potent virus specific clonotypes. However, the tool can be adapted to model other barcoded time series frequency data and the accompanying vignette demonstrates how the tool could be used to track clonotypes *in vivo*, using Adaptive's ImmuneAccess database. 
