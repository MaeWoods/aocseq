...
The TCR sequence is important to study because the specificity of a T cell depends on a variable region of the TCR, known as the **CDR3**. T cells can be grouped into clonotypes that share a common CDR3 beta sequence and this way, used to approximate the potential frequency of target specific T cells in the blood. Harnessing the heterogenous sequence between T cells for a quantitative analysis of T cell immunology has broader application to *in vivo* data analysis because the immune response to a pathogen depends on availability of immune cells with capacity to mount a response against infection and the number of target specific T cells contributes to this response. 
