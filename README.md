...
The aim of the tool is to provide a suite of statistical methods based on control theory for the classification and scoring of cells with optimal characteristics. Characteristics are defined by the gene expression of cells that are considered desirable by their response to perturbation. The current version of the software can be used to quantify cells that are activated through the binding of their T cell receptor (TCR) to its cognate antigen. Therefore the tool provides a method to identify full length TCRs with specfifity directed towards a target antigen, that can then be used as transgenes. In a later update, functionality for time series analysis will be included, whereby the CDR3 TCRbeta dataset of a TCR repertoire in the format of nucleic acid sequence productive frequency, the corresponding amino acid sequences and the total number of T cells at each time point will be required. Currently, the data can be from multiple conditions or treatments, hashtagged and include multiple time points. 
